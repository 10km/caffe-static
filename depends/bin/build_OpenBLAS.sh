...
make -j $MAKE_JOBS CC=$MAKE_C_COMPILER NOFORTRAN=1 NO_LAPACKE=1 NO_SHARED=1
if [  $? -ne 0 ]
then
		make clean
		exit_on_error
		# 如果编译出错，加上NO_AVX2=1再尝试一次,解决Error: no such instruction: `vpermpd` 
		make -j $MAKE_JOBS CC=$MAKE_C_COMPILER NOFORTRAN=1 NO_LAPACKE=1 NO_SHARED=1 NO_AVX2=1
		exit_on_error
fi
